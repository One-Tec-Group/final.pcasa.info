...
class Dashboard extends Admin_Controller {
    ...
    public function index($action = NULL) {
        ...
        $this->admin_model->_table_name = "tbl_transactions";
        $this->admin_model->_order_by = "transactions_id";
        $total_income_expense = $this->admin_model->get();
        $today_income_expense = $this->admin_model->get_by(array('date' => date('Y-m-d')));
    ...
    public function finance_overview_by_year($year, $annual = null) {
        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            $finance_overview_list[$i] = $this->report_model->get_report_by_date($start_date, $end_date);
        }
        if (!empty($annual)) {
            $f_total_expense = 0;
            $f_total_income = 0;
    ...
    public function get_goal_report($month) {
        ...
        $end_date = $year . "-" . $m . '-' . '31';
        $get_goal_report = $this->admin_model->get_goal_report_by_month($start_date, $end_date);
    ...
    public function get_income_expense($month) {
        ...
        $get_expense_list = $this->admin_model->get_transactions_list_by_month($start_date, $end_date);
    ...
    public function get_transactions_list($year, $type) {
        for ($i = 1; $i <= 12; $i++) {
            ...
            $get_expense_list[$i] = $this->admin_model->get_transactions_list_by_date($type, $start_date, $end_date);
        }
        return $get_expense_list;
    }
    ...
    public function set_clocking($id = NULL, $user_id = null, $row = null, $redirect = null) {
        if (!empty(attendance_access())) {
            ...
            $this->admin_model->_table_name = "tbl_clock";
            $this->admin_model->_primary_key = "clock_id";
            if (!empty($id)) {
    ...
    public function save_todo($id = null) {
        ...
        $this->admin_model->_table_name = "tbl_todo";
        $this->admin_model->_primary_key = "todo_id";
        $this->admin_model->save($data, $id);
        $type = "success";
    ...
    public function social_platform(){
      $can_do = can_do(162);
      if (!empty($can_do)) {
        echo '<form id="socialPlaatformLogin" method="POST" action="' . base_url() . 'marketing/sp/home/login">
              <input type="hidden" name="username" value="a.ayad@pcasa.ae"/>
              <input type="hidden" name="password" value="Ayad1234"/>
              <input type="hidden" name="backURLFromMarketting" value="' . base_url("admin/dashboard") . '"/>
              <input type="hidden" name="FromName" value="CRM"/>
              </form><script>document.getElementById("socialPlaatformLogin").submit();</script>';
        exit();
      }
    }
