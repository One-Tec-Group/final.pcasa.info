...
class Client extends Admin_Controller {
    ...
    public function paymentsList($id) {
        if ($this->input->is_ajax_request()) {
            ...
            $fetch_data = $this->db->where('paid_by', $id)->order_by('created_date', 'desc')->get('tbl_payments')->result();
            $data = array();
        ...
        if (!empty($client_contacts)) {
            foreach ($client_contacts as $contact) {
                ...
                array_push($root_options['attributes'], array(
                    'pattern' => '/.(' . $contact->media_path_slug . '+)/',
                    'read' => true,
                    'write' => true,
                ));
        ...
        if (!empty($id)) {
            ...
            if (!empty($u_data['password']) && !empty($u_data['username']) && !empty($u_data['email'])) {
                ...
                if ($this->input->post('send_email_password', true) == 'on') {
                    $this->send_confirmation_email($u_data, $user_data['password']);
                }
                $activities = array(
                    'user' => $this->session->userdata('user_id'),
                    'module' => 'client',
    ...
    public function delete_client($client_id, $yes = null) {
        $deleted = can_action('4', 'deleted');
            ...
            if (!empty($deleted)) {
                ...
                if (!empty($project_info)) {
                    foreach ($project_info as $v_project) {
                        $this->client_model->_table_name = "tbl_task_comment";
                        $this->client_model->delete_multiple(array('project_id' => $v_project->project_id));
                        $this->client_model->_table_name = "tbl_task_attachment";
                        ...
                        if (!empty($files_info)) {
                            foreach ($files_info as $v_files) {
                                $this->client_model->_table_name = "tbl_task_uploaded_files";
                                $this->client_model->delete_multiple(array('task_attachment_id' => $v_files->task_attachment_id));
                            }
                        }
                        $this->client_model->_table_name = "tbl_task_attachment";
                        $this->client_model->delete_multiple(array('project_id' => $v_project->project_id));
                        $this->client_model->_table_name = "tbl_milestones";
                        $this->client_model->delete_multiple(array('project_id' => $v_project->project_id));
                        ...
                        if (!empty($taskss_info)) {
                            foreach ($taskss_info as $v_taskss) {
                                ...
                                $this->client_model->_table_name = "tbl_task_comment";
                                $this->client_model->delete_multiple(array('task_id' => $v_taskss->task_id));
                                $this->client_model->_table_name = "tbl_task";
                                $this->client_model->_primary_key = "task_id";
                        ...
                        if (!empty($bugs_info)) {
                            foreach ($bugs_info as $v_bugs) {
                                ...
                                $this->client_model->_table_name = "tbl_task";
                                $this->client_model->delete_multiple(array('bug_id' => $v_bugs->bug_id));
                                $this->client_model->_table_name = "tbl_bug";
                                $this->client_model->_primary_key = "bug_id";
    ...
    public function opportunityConverted($leads_id) {
        $data = $this->items_model->array_from_post(array('lead_id', 'opportunity_id' , 'name', 'email', 'short_note', 'website', 'phone', 'mobile', 'fax', 'address', 'city', 'zipcode', 'currency',
            'skype_id', 'linkedin', 'facebook', 'twitter', 'language', 'country', 'client_status', 'latitude', 'longitude', 'customer_group_id'));
        if (!empty($_FILES['profile_photo']['name'])) {
