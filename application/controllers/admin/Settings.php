...
//            }
    ...
    public function properties_source($action = NULL, $id = NULL)
    {
        $created = can_action('158', 'created');
        $edited = can_action('158', 'edited');
        $data['page'] = lang('settings');
        if ($action == 'edit_property_source') {
            $data['active'] = 2;
            if (!empty($id) && !empty($edited)) {
                $data['property_source_info'] = $this->settings_model->check_by(array('property_source_id' => $id), 'tbl_property_source');
            }
        } else {
            $data['active'] = 1;
        }
        $data['page'] = lang('settings');
        $data['sub_active'] = lang('proeprties_source');
        if ($action == 'update_property_source') {
            if (!empty($created) || !empty($edited) && !empty($id)) {
                $this->settings_model->_table_name = 'tbl_property_source';
                $this->settings_model->_primary_key = 'property_source_id';

                $source_data['property_source'] = $this->input->post('property_source', TRUE);
                // update root category
                $where = array('property_source' => $source_data['property_source']);
                // duplicate value check in DB
                if (!empty($id)) { // if id exist in db update data
                    $proeprty_source_id = array('property_source_id !=' => $id);
                } else { // if id is not exist then set id as null
                    $proeprty_source_id = null;
                }
                // check whether this input data already exist or not
                $check_proeprty_status = $this->settings_model->check_update('tbl_property_source', $where, $property_source_id);
                if (!empty($check_proeprty_status)) { // if input data already exist show error alert
                    // massage for user
                    $type = 'error';
                    $msg = "<strong style='color:#000'>" . $source_data['property_source'] . '</strong>  ' . lang('already_exist');
                } else { // save and update query
                    $id = $this->settings_model->save($source_data, $id);
                    ...
                    $activity = array(
                        ...
                        'module' => 'settings',
                        'module_field_id' => $id,
                        'activity' => ('activity_added_a_property_source'),
                        'value1' => $source_data['proeprty_source']
                    );
                    $this->settings_model->_table_name = 'tbl_activities';
                    $this->settings_model->_primary_key = 'activities_id';
                    $this->settings_model->save($activity);
                    ...
                    // messages for user
                    $type = "success";
                    $msg = lang('properties_source_added');
                }
                $message = $msg;
                set_message($type, $message);
            }
            redirect('admin/settings/properties_source');
        } else {
            $data['title'] = lang('proeprties_source');
            $data['load_setting'] = 'properties_source';
        }

        $this->settings_model->_table_name = 'tbl_property_source';
        $this->settings_model->_order_by = 'property_source_id';
        $data['all_property_source'] = $this->settings_model->get();
    ...
    public function delete_property_source($id)
    {
        $deleted = can_action('158', 'deleted');
        if (!empty($deleted)) {
            $property_source = $this->settings_model->check_by(array('property_source_id' => $id), 'tbl_property_source');
            $activity = array(
                'user' => $this->session->userdata('user_id'),
                'module' => 'settings',
                'module_field_id' => $id,
                'activity' => ('activity_delete_a_property_source'),
                'value1' => $property_source->property_source,
            );
            $this->settings_model->_table_name = 'tbl_activities';
            $this->settings_model->_primary_key = 'activities_id';
            $this->settings_model->save($activity);
            ...
            // messages for user
            $type = "success";
            $message = lang('proeprty_source_deleted');
            // messages for user
            echo json_encode(array("status" => $type, 'message' => $message));
            exit();
        } else {
    ...
    public function properties_type($action = NULL, $id = NULL)
    {
        $created = can_action('159', 'created');
        $edited = can_action('159', 'edited');
        $data['page'] = lang('settings');
        if ($action == 'edit_property_type') {
            $data['active'] = 2;
            if (!empty($id) && !empty($edited)) {
                $data['property_type_info'] = $this->settings_model->check_by(array('property_type_id' => $id), 'tbl_properties_types');
            }
        } else {
            $data['active'] = 1;
        }
        $data['page'] = lang('settings');
        $data['sub_active'] = lang('proeprties_type');
        if ($action == 'update_property_type') {
            if (!empty($created) || !empty($edited) && !empty($id)) {
                $this->settings_model->_table_name = 'tbl_properties_types';
                $this->settings_model->_primary_key = 'property_type_id';

                $type_data['property_type'] = $this->input->post('property_type', TRUE);
                // update root category
                $where = array('property_type' => $type_data['property_type']);
                // duplicate value check in DB
                if (!empty($id)) { // if id exist in db update data
                    $proeprty_type_id = array('property_type_id !=' => $id);
                } else { // if id is not exist then set id as null
                    $proeprty_type_id = null;
                }
                // check whether this input data already exist or not
                $check_proeprty_status = $this->settings_model->check_update('tbl_properties_types', $where, $proeprty_type_id);
                if (!empty($check_proeprty_status)) { // if input data already exist show error alert
                    // massage for user
                    $type = 'error';
                    $msg = "<strong style='color:#000'>" . $type_data['property_type'] . '</strong>  ' . lang('already_exist');
                } else { // save and update query
                    $id = $this->settings_model->save($type_data, $id);

                    $activity = array(
                        'user' => $this->session->userdata('user_id'),
                        'module' => 'settings',
                        'module_field_id' => $id,
                        'activity' => ('activity_added_a_property_type'),
                        'value1' => $type_data['proeprty_type']
                    );
                    $this->settings_model->_table_name = 'tbl_activities';
                    $this->settings_model->_primary_key = 'activities_id';
                    $this->settings_model->save($activity);
                    ...
                    // messages for user
                    $type = "success";
                    $msg = lang('properties_type_added');
                }
                $message = $msg;
                set_message($type, $message);
            }
            redirect('admin/settings/properties_type');
        } else {
            $data['title'] = config_item('company_name') . ' | ' . lang('proeprties_type');
            $data['load_setting'] = 'properties_type';
        }

        $this->settings_model->_table_name = 'tbl_properties_types';
        $this->settings_model->_order_by = 'property_type_id';
        $data['all_property_type'] = $this->settings_model->get();
    ...
    public function delete_property_type($id)
    {
        ...
        if (!empty($deleted)) {
            $property_type = $this->settings_model->check_by(array('property_type_id' => $id), 'tbl_properties_types');
            $activity = array(
                'user' => $this->session->userdata('user_id'),
                'module' => 'settings',
                'module_field_id' => $id,
                'activity' => ('activity_delete_a_property_type'),
                'value1' => $property_type->property_type,
            );
            $this->settings_model->_table_name = 'tbl_activities';
            $this->settings_model->_primary_key = 'activities_id';
            $this->settings_model->save($activity);
            ...
            // messages for user
            $type = "success";
            $message = lang('proeprty_type_deleted');
            // messages for user
            echo json_encode(array("status" => $type, 'message' => $message));
            exit();
        } else {
