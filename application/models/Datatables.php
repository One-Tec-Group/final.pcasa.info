...
class Datatables extends MY_Model
{
    ...
    function make_query()
    {
        ...
        $this->db->from($this->table);
          ...
          if(in_array($this->uri->segment(2), array("leads", "properties"))){
              $join = $this->join_where[1];
              $filters = $this->join_where[0];
              if (!empty($join)) {
...
//        foreach ($all_projects as $v_projects) {
    ...
    {
        $all_leads_info = array_reverse($this->get_datatable_permission());
    ...
    {
        $all_properties_info = array_reverse($this->get_datatable_permission());
