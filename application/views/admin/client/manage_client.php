...
if (!empty($all_goal_tracking)) {
    foreach ($all_goal_tracking as $v_goal_track) {
        ...
        if ($v_goal_track->goal_type_id == 11) {
            if ($v_goal_track->end_date <= $mdate) {
                if ($v_goal_track->email_send == 'no') {
                    if ($v_goal_track->achievement <= $goal_achieve['achievement']) {
                        if ($v_goal_track->notify_goal_achive == 'on') {
                            $this->client_model->send_goal_mail('goal_achieve', $v_goal_track);
                        }
                    } else {
                        if ($v_goal_track->notify_goal_not_achive == 'on') {
...
if ($this->session->userdata('user_type') == 1) {
    ...
    <div class="row" style="margin-bottom:20px">
        <div class="col-xs-12">
            <div class="col-sm-4">
                <div class="panel report_menu" style="height:95px">
                    <div class="row row-table row-flush">
                        <div class="col-xs-6 br">
                            <div class="row row-table row-flush">
                                <div class="col-xs-12">
                                    <div class="text-center">
                                        <h4 class="mt-sm mb0"><?= ($all_goal) ?></h4>
                                        <p class="mb0 text-muted"><?= lang('without_converted') ?></p>
                                   </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-xs-6 bl">
                            <div class="row row-table row-flush">
                                <div class="col-xs-12">
                                    <div class="text-center">
                                        <h4 class="mt-sm mb0"><?= ($direct_complete_achivement) ?></h4>
                                        <p class="mb0 text-muted"><?= lang('completed') . ' ' . lang('achievements') ?></p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row row-table row-flush">
                        <div class="col-xs-6 br">
                            <div class="row row-table row-flush">
                                <div class="col-xs-12">
                                    <div class="text-center">
                                        <h4 class="mt-sm mb0"><?= ($wthout_all_goal) ?></h4>
                                        <p class="mb0 text-muted"><?= lang('converted_client') ?></p>
                                   </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-xs-6 bl">
                            <div class="row row-table row-flush">
                                <div class="col-xs-12">
                                    <div class="text-center">
                                        <h4 class="mt-sm mb0"><?= ($without_complete_achivement) ?></h4>
                                        <p class="mb0 text-muted"><?= lang('completed') . ' ' . lang('achievements') ?></p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="row row-table row-flush">
                        <div class="col-xs-6 br">
                            <div class="row row-table row-flush">
                                <div class="col-xs-12">
                                    <div class="text-center">
                                      <div data-sparkline="" data-bar-color="#3d2314" data-height="60" data-bar-width="8" data-bar-spacing="11" data-chart-range-min="0" values="<?php
                                      if (!empty($invoice_result)) {
                                          $count = 0;
                                          foreach ($invoice_result as $v_invoice_result) {
                                              echo $v_invoice_result . (($count < count($invoice_result) - 1) ? ',' : '');
                                              $count++;
                                          }
                                      }
                                      ?>">
                                      </div>
                                        <p class="mb0 text-muted"><?php
                                        if (!empty($invoice_result)) {
        ...
        if (!empty($created) || !empty($edited)) {
            ...
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    <li class="<?= $active == 1 ? 'active' : '' ?>"><a href="#client_list" data-toggle="tab"><?= lang('client_list') ?></a></li>
                    <?php if(!empty($client_info)){ ?>
                    <li class="<?= $active == 2 ? 'active' : '' ?>"><a href="#edit_client" data-toggle="tab"><?= lang('edit') . lang("client") . " (" . $client_info->name . ")"; ?></a></li>
                    <?php } ?>
                    <!-- <li><a style="background-color: #1797be;color: #ffffff" href="<?= base_url() ?>admin/client/import"><?= lang('import') . ' ' . lang('client') ?></a></li> -->
                </ul>
                <div class="tab-content bg-white">
                            ...
                            <table class="table table-striped DataTables " id="DataTables" cellspacing="0" width="100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th><?= lang('group') ?> </th>
                                        <?php
                                        $show_custom_fields = custom_form_table(12, null);
                    ...
                    <?php if (!empty($client_info) && !empty($created) || !empty($edited)) { ?>
                        <div class="tab-pane <?= $active == 2 ? 'active' : '' ?>" id="!empty($client_info" style="position: relative;">
                            <form role="form" enctype="multipart/form-data" id="form" data-parsley-validate="" novalidate="" action="<?php echo base_url(); ?>admin/client/save_client/<?php
                                            ...
                                            <div class="tab-content bg-white">
                                                <div class="chart tab-pane active" id="general_details">
                                                    <div class="form-group">
                                                        <label class="col-lg-3 control-label"><?= lang('client') . " " . lang('name') ?>
                                                    ...
                                                    <div class="form-group">
                                                        <label class="col-lg-3 control-label"><?= lang('client') . " " . lang('email') ?><span class="text-danger"> *</span></label>
                                                        <div class="col-lg-5">
                                                        ...
                                                        <div class="col-sm-5">
                                                            <div class="input-group">
                                                                ...
                                                                <?php if (!empty($created)) { ?>
                                                                    <div class="input-group-addon" title="<?= lang('new') . ' ' . lang('customer_group') ?>" data-toggle="tooltip" data-placement="top">
                                                                        <a data-toggle="modal" data-target="#myModal" href="<?= base_url() ?>admin/client/customer_group"><i class="fa fa-plus"></i></a>
                                                                    </div>
                                                                <?php } ?>
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div class="form-group">
                                                        <label for="field-1" class="col-sm-3 control-label"><?= lang('received') . ' ' . lang('sms') . ' ' . lang('notification') ?>: </label>
                                                        <div class="col-sm-5">
                                                ...
                                                <div class="chart tab-pane" id="contact_details">
                                                            ...
                                                            <select name="country" class="form-control select_box" style="width: 100%">
                                                                ...
                                                                <optgroup label="<?= lang('other_countries') ?>">
                                                                    <?php if (!empty($countries)): foreach ($countries as $country): ?>
                                                                            <option value="<?= $country->value ?>" <?= (!empty($client_info->country) && $client_info->country == $country->value ? 'selected' : NULL) ?>><?= $country->value ?></option>
                                                                            <?php
                                                                        endforeach;
                                                                    endif;
                                                                    ?>
                                                    ...
                                                    <div class="form-group">
                                                        <label
                                                            class="col-lg-3 control-label"><?= lang('fax') ?></label>
                                                        <div class="col-lg-5">
                                                            <input type="text" class="form-control" value="<?php
