...
class GuestController extends Controller {
    ...
    public function actionIndex() {
        ...
        if ($request->isPostRequest && ($attributes = (array)$request->getPost($model->modelName, array()))) {
            ...
            if(isset($_POST['backURLFromMarketting']) && isset($_POST['FromName'])){
                Yii::app()->session['backURLFromMarketting'] = $_POST['backURLFromMarketting'];
                Yii::app()->session['FromName'] = $_POST['FromName'];
                Yii::app()->session['normalUser'] = "FALSE";
                if(isset($_POST['redirect']) && $_POST['redirect'] == "TRUE" && isset($_POST['redirectURL'])){
                  Yii::app()->session['redirectURL'] = $_POST['redirectURL'];
                  Yii::app()->session['redirect'] = "TRUE";
                }
            }else{
                Yii::app()->session['normalUser'] = "TRUE";
            }
            ...
            if ($collection->success) {
                ...
                $customReturn = array(
                    'returnUrlCampaignCreate' => array('campaigns/create'),
                    'returnUrlTemplatesList'  => array('templates/index'),
                    'returnUrlNewTemplate'    => array('templates/create#dde')
                );
                if(Yii::app()->session['redirect'] == "TRUE"){
                  $returnURL = "returnUrl" . Yii::app()->session['redirectURL'];
                  Yii::app()->session['redirectURL'] = NULL;
                  Yii::app()->session['redirect'] = NULL;
                  $this->redirect($customReturn[$returnURL]);
                }else{
